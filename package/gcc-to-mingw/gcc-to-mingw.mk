...
GCC_TO_MINGW_VERSION = $(call qstrip,$(BR2_GCC_VERSION))

ifeq ($(BR2_GCC_VERSION_ARC),y)
GCC_TO_MINGW_SITE = $(call github,foss-for-synopsys-dwc-arc-processors,gcc,$(GCC_TO_MINGW_VERSION))
GCC_TO_MINGW_SOURCE = gcc-$(GCC_TO_MINGW_VERSION).tar.gz
else
GCC_TO_MINGW_SITE = $(BR2_GNU_MIRROR:/=)/gcc-to-mingw/gcc-$(GCC_TO_MINGW_VERSION)
GCC_TO_MINGW_SOURCE = gcc-$(GCC_TO_MINGW_VERSION).tar.xz
endif

#
# Xtensa special hook
#
define HOST_GCC_TO_MINGW_XTENSA_OVERLAY_EXTRACT
...
define HOST_GCC_TO_MINGW_APPLY_PATCHES
	for patchdir in \
	...
	$(HOST_GCC_TO_MINGW_APPLY_POWERPC_PATCH)
endef

HOST_GCC_TO_MINGW_EXCLUDES = \
	libjava/* libgo/*
...
define HOST_GCC_TO_MINGW_CONFIGURE_SYMLINK
	mkdir -p $(@D)/build
...
HOST_GCC_TO_MINGW_COMMON_DEPENDENCIES = \
	host-binutils-to-mingw \
	host-gmp \
	host-mpc \
	host-mpfr \
...
HOST_GCC_TO_MINGW_COMMON_CONF_OPTS += --with-debug-prefix-map=$(BASE_DIR)=buildroot
endif
...
GCC_TO_MINGW_COMMON_TARGET_CFLAGS = $(TARGET_CFLAGS)
GCC_TO_MINGW_COMMON_TARGET_CXXFLAGS = $(TARGET_CXXFLAGS)
...
GCC_TO_MINGW_COMMON_TARGET_CFLAGS += -Wno-error
endif

# Make sure libgcc & libstdc++ always get built with -matomic on ARC700
ifeq ($(GCC_TO_MINGW_TARGET_CPU):$(BR2_ARC_ATOMIC_EXT),arc700:y)
GCC_TO_MINGW_COMMON_TARGET_CFLAGS += -matomic
GCC_TO_MINGW_COMMON_TARGET_CXXFLAGS += -matomic
endif

# Propagate options used for target software building to GCC target libs
HOST_GCC_TO_MINGW_COMMON_CONF_ENV += CFLAGS_FOR_TARGET="$(GCC_TO_MINGW_COMMON_TARGET_CFLAGS)"
HOST_GCC_TO_MINGW_COMMON_CONF_ENV += CXXFLAGS_FOR_TARGET="$(GCC_TO_MINGW_COMMON_TARGET_CXXFLAGS)"
HOST_GCC_TO_MINGW_COMMON_CONF_ENV += AR_FOR_TARGET=gcc-ar NM_FOR_TARGET=gcc-nm RANLIB_FOR_TARGET=gcc-ranlib

# libitm needs sparc V9+
#ifeq ($(BR2_sparc_v8)$(BR2_sparc_leon3),y)
#HOST_GCC_TO_MINGW_COMMON_CONF_OPTS += --disable-libitm
#endif

# libmpx uses secure_getenv and struct _libc_fpstate not present in musl
#ifeq ($(BR2_TOOLCHAIN_BUILDROOT_MUSL)$(BR2_TOOLCHAIN_GCC_TO_MINGW_AT_LEAST_6),yy)
#HOST_GCC_TO_MINGW_COMMON_CONF_OPTS += --disable-libmpx
#endif

# quadmath support requires wchar
ifeq ($(BR2_USE_WCHAR)$(BR2_TOOLCHAIN_HAS_LIBQUADMATH),yy)
HOST_GCC_TO_MINGW_COMMON_CONF_OPTS += --enable-libquadmath --enable-libquadmath-support
else
HOST_GCC_TO_MINGW_COMMON_CONF_OPTS += --disable-libquadmath --disable-libquadmath-support
endif

# libsanitizer requires wordexp, not in default uClibc config. Also
# doesn't build properly with musl.
ifeq ($(BR2_TOOLCHAIN_BUILDROOT_UCLIBC)$(BR2_TOOLCHAIN_BUILDROOT_MUSL),y)
HOST_GCC_TO_MINGW_COMMON_CONF_OPTS += --disable-libsanitizer
endif
...
#ifeq ($(BR2_mips)$(BR2_mipsel):$(BR2_TOOLCHAIN_GCC_TO_MINGW_AT_LEAST_12),y:y)
#HOST_GCC_TO_MINGW_COMMON_CONF_OPTS += --disable-libsanitizer
#endif
...
HOST_GCC_TO_MINGW_COMMON_CONF_ENV += target_configargs="libbacktrace_cv_sys_sync=no"
endif
...
HOST_GCC_TO_MINGW_COMMON_CONF_OPTS += --disable-tls
else
HOST_GCC_TO_MINGW_COMMON_CONF_OPTS += --enable-tls
endif
...
HOST_GCC_TO_MINGW_COMMON_CONF_OPTS += --enable-threads
endif
...
ifeq ($(BR2_GCC_TO_MINGW_ENABLE_GRAPHITE),y)
HOST_GCC_TO_MINGW_COMMON_DEPENDENCIES += host-isl
HOST_GCC_TO_MINGW_COMMON_CONF_OPTS += --with-isl=$(HOST_DIR)
else
HOST_GCC_TO_MINGW_COMMON_CONF_OPTS += --without-isl --without-cloog
endif

ifeq ($(BR2_arc),y)
HOST_GCC_TO_MINGW_COMMON_DEPENDENCIES += host-flex host-bison
endif
...
#HOST_GCC_TO_MINGW_COMMON_CONF_OPTS += --with-float=soft
#endif
#endif

# Determine arch/tune/abi/cpu options
#ifneq ($(GCC_TO_MINGW_TARGET_ARCH),)
#HOST_GCC_TO_MINGW_COMMON_CONF_OPTS += --with-arch="$(GCC_TO_MINGW_TARGET_ARCH)"
#endif
#ifneq ($(GCC_TO_MINGW_TARGET_ABI),)
#HOST_GCC_TO_MINGW_COMMON_CONF_OPTS += --with-abi="$(GCC_TO_MINGW_TARGET_ABI)"
#endif
#ifeq ($(BR2_TOOLCHAIN_HAS_MNAN_OPTION),y)
#ifneq ($(GCC_TO_MINGW_TARGET_NAN),)
#HOST_GCC_TO_MINGW_COMMON_CONF_OPTS += --with-nan="$(GCC_TO_MINGW_TARGET_NAN)"
#endif
#endif
#ifneq ($(GCC_TO_MINGW_TARGET_FP32_MODE),)
#HOST_GCC_TO_MINGW_COMMON_CONF_OPTS += --with-fp-32="$(GCC_TO_MINGW_TARGET_FP32_MODE)"
#endif
#ifneq ($(GCC_TO_MINGW_TARGET_CPU),)
#HOST_GCC_TO_MINGW_COMMON_CONF_OPTS += --with-cpu=$(GCC_TO_MINGW_TARGET_CPU)
#endif

#ifneq ($(GCC_TO_MINGW_TARGET_FPU),)
#HOST_GCC_TO_MINGW_COMMON_CONF_OPTS += --with-fpu=$(GCC_TO_MINGW_TARGET_FPU)
#endif

#ifneq ($(GCC_TO_MINGW_TARGET_FLOAT_ABI),)
#HOST_GCC_TO_MINGW_COMMON_CONF_OPTS += --with-float=$(GCC_TO_MINGW_TARGET_FLOAT_ABI)
#endif

#ifneq ($(GCC_TO_MINGW_TARGET_MODE),)
#HOST_GCC_TO_MINGW_COMMON_CONF_OPTS += --with-mode=$(GCC_TO_MINGW_TARGET_MODE)
#endif
...
#HOST_GCC_TO_MINGW_COMMON_CONF_OPTS += --enable-secureplt
#endif
...
#ifeq ($(BR2_TOOLCHAIN_USES_GLIBC)$(BR2_TOOLCHAIN_GCC_TO_MINGW_AT_LEAST_6)$(BR2_powerpc64le),yyy)
#HOST_GCC_TO_MINGW_COMMON_CONF_OPTS += \
#	--with-long-double-128
#endif
...
HOST_GCC_TO_MINGW_COMMON_TOOLCHAIN_WRAPPER_ARGS += -DBR_CROSS_PATH_SUFFIX='".br_real"'
...
HOST_GCC_TO_MINGW_COMMON_MAKE_OPTS = \
	gcc_cv_libc_provides_ssp=$(if $(BR2_TOOLCHAIN_HAS_SSP),yes,no)

ifeq ($(BR2_CCACHE),y)
HOST_GCC_TO_MINGW_COMMON_CCACHE_HASH_FILES += $($(PKG)_DL_DIR)/$(GCC_TO_MINGW_SOURCE)
...
HOST_GCC_TO_MINGW_COMMON_CCACHE_HASH_FILES += \
	$(sort $(wildcard \
		package/gcc-to-mingw/$(GCC_TO_MINGW_VERSION)/*.patch \
		$(addsuffix /$($(PKG)_RAWNAME)/$(GCC_TO_MINGW_VERSION)/*.patch,$(call qstrip,$(BR2_GLOBAL_PATCH_DIR))) \
		$(addsuffix /$($(PKG)_RAWNAME)/*.patch,$(call qstrip,$(BR2_GLOBAL_PATCH_DIR))) \
		$(addsuffix /gcc-to-mingw/$(GCC_TO_MINGW_VERSION)/*.patch,$(call qstrip,$(BR2_GLOBAL_PATCH_DIR))) \
		$(addsuffix /gcc-to-mingw/*.patch,$(call qstrip,$(BR2_GLOBAL_PATCH_DIR)))))
ifeq ($(BR2_xtensa),y)
HOST_GCC_TO_MINGW_COMMON_CCACHE_HASH_FILES += $(ARCH_XTENSA_OVERLAY_FILE)
endif
...
HOST_GCC_TO_MINGW_COMMON_TOOLCHAIN_WRAPPER_ARGS += -DBR_CCACHE_HASH=\"`\
	printf '%s\n' $(subst $(HOST_DIR),@HOST_DIR@,\
		$(subst --with-pkgversion="Buildroot $(BR2_VERSION_FULL)",,$($(PKG)_CONF_OPTS))) \
		| sha256sum - $(HOST_GCC_TO_MINGW_COMMON_CCACHE_HASH_FILES) \
		| cut -c -64 | tr -d '\n'`\"
endif # BR2_CCACHE
...
#define HOST_GCC_TO_MINGW_INSTALL_WRAPPER_AND_SIMPLE_SYMLINKS
#	$(Q)cd $(HOST_DIR)/bin; \
...
include $(sort $(wildcard package/gcc-to-mingw/*/*.mk))
